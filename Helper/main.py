...
class SympyApi:
    ...
    def method(self, method_name, method_types=None, access_code=None):
        ''' '''
    ...
    def check_access(self, function):
        def decorator():
            self._check_access = function
        return decorator

    def get_access_code(self, token_arg='token'):
        def decorator(function):
        ...
        try:
            ...
            if error.name in self.exceptions.methods:
                return self.exceptions.get(error.name)(
                    method_name, *error.args, **error.kwargs)
            else:
                raise
